...
namespace Cic.OpenOne.Common.Util.Security
{
    /// <summary>
    /// Blowfish-Klasse
    /// DB Passwort decryption Algorithm
    /// </summary>
    public sealed class Blowfish
    {
        ...
        private const int MAX_PKEY = 56;  //  ! 448bits
        #endregion

        #region Private variables

        // Initial vectors
        private uint[] _InitialPB = 
		{
        ...
        public Blowfish(string key)
        {
            ...
            if (key.Length > MAX_PKEY)
            {
                throw new ArgumentOutOfRangeException("key", "Max = " + MAX_PKEY);
            }

            // Get key bytearray
            Key = MyGetBytesArray(key);

            _PB = _InitialPB.Clone() as uint[];
            _SB = _InitialSB.Clone() as uint[][];
            ...
            {
                MyBlowfishEncode(ref datal, ref datar);
                _PB[i] = datal;
                _PB[i + 1] = datar;
            }
        ...
        public string Encode(string data)
        {
            ...
            word = MyGetBytesArray(data);
            length = data.Length;

            MyAddTrailingZeros(ref word, ref length);

            // Encode
            MyEncode(word, length);

            return MyGetString(word);
        }
        ...
        public string Decode(string data)
        {
            ...
            Word = MyGetBytesArrayFromHex(data);
            Length = Word.Length;

            // Decode
            MyDecode(Word, Length);

            // Remove trailing 0
            Word = MyRemoveTrailingZeros(Word);
            
            return System.Text.Encoding.UTF7.GetString(Word);
            //return AsciiEncoding.GetString(Word);
        }
        #endregion

        #region My methods

        private static void MyAddTrailingZeros(ref byte[] word, ref int length)
        {
            // Resize array to multiplicity of PAD
            // resize from 0 to 8, from 8 to 16, and so on - clarion :(
            length = length + (PAD - (length % PAD));
            System.Array.Resize<byte>(ref word, length);
        }

        private static byte[] MyRemoveTrailingZeros(byte[] Word)
        {
            System.Collections.Generic.List<byte> TrailedWord;
            int Counter;

            TrailedWord = new System.Collections.Generic.List<byte>(Word);
            TrailedWord.Reverse();

            Counter = 0;
            foreach (byte ByteLoop in TrailedWord)
            {
                if (ByteLoop != 0)
                {
                    break;
            ...
            TrailedWord.RemoveRange(0, Counter);
            TrailedWord.Reverse();

            return TrailedWord.ToArray();
        }
        ...
        private static byte[] MyGetBytesArrayFromHex(string data)
        {
            System.Collections.Generic.List<System.Byte> byteList = new System.Collections.Generic.List<byte>();

            for (int i = 0; i < data.Length; i = i + 2)
            {
                byteList.Add(System.Convert.ToByte(data.Substring(i, 2), 16));
            }
            return byteList.ToArray();
        }

        // Gets hex string from byte array
        private static string MyGetString(byte[] data)
        {
            ...
            {
                // Convert to hex
                byteString = sign.ToString("X", System.Globalization.CultureInfo.InvariantCulture);
                // Add leading 0
                if (byteString.Length == 1) byteString = "0" + byteString;
                str.Append(byteString);
            }
            return str.ToString();
        }

        private void MyEncode(byte[] data, int length)
        {
            uint xl, xr;
            ...
            for (int i = 0; i < length; i += 8)
            {
                ...
                MyBlowfishEncode(ref xl, ref xr);
        ...
        private void MyDecode(byte[] data, int length)
        {
            uint xl, xr;
            if ((length % PAD) != 0)
                // TODO MK 0 BK, Add exception class
            ...
            for (int i = 0; i < length; i += 8)
            {
                ...
                xl = (uint)((data[i + 3] << 24) | (data[i + 2] << 16) | (data[i + 1] << 8) | data[i]);
                xr = (uint)((data[i + 7] << 24) | (data[i + 6] << 16) | (data[i + 5] << 8) | data[i + 4]);
                MyBlowfishDecode(ref xl, ref xr);
        ...
        private void MyBlowfishEncode(ref uint xl, ref uint xr)
        {
            uint Xl;
            uint Xr;
            uint temp;
        ...
        private void MyBlowfishDecode(ref uint xl, ref uint xr)
        {
            uint Xl;
            uint Xr;
            uint temp;
