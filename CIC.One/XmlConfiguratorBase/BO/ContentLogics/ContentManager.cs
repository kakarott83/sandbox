...
namespace XmlConfiguratorBase.BO.ContentLogics
{
    ...
    public class ContentManager
    {
        ...
        public ContentManager ()
        {
            ...
            log.Debug("ContentManager construct");
            ...
            Mapper.CreateMap<WfvConfigEntry, WfvEntry>()
                .ForMember(dest => dest.entrytype, opt => opt.MapFrom(entry => (entry == null || entry.entrytype == null) ? 0 : int.Parse(entry.entrytype)))
                .ForAllMembers(opt => opt.Condition(src => src != null && src.DestinationValue == null && !src.IsSourceValueNull && (!(src.SourceValue is string) || ((string)src.SourceValue).Length > 0)))
                ;
        ...
        {
            log.Debug("ContentManager ResetData");
            syscodes.Clear();
        ...
        {
            log.Debug("ContentManager CalculateNewItems");
            if (ContentData == null)
                ContentData = data;
            else if (data != null)
            {
                log.Debug("Anzahl: " + ContentData.entries == null ? "null" : ContentData.entries.Count + " C entries, " + ContentData.configentries == null ? "null" : ContentData.configentries.Count + " C config, " + data.entries == null ? "null" : data.entries.Count + " D entries, " + data.configentries == null ? "null" : data.configentries.Count + " D config");
        ...
        public List<WfvConfigEntryDto> GetEntryConfigList()
        {
            ...
            if (ContentData != null)
            {
                ...
                foreach (WfvConfigEntry xmlEntry in ContentData.configentries)
                {
                    ...
                    entry.einrichtung = GetEinrichtungObject(xmlEntry);
                    if (entry.einrichtung is WfvEntry)
                    {
        ...
        /// <summary>
        /// Turn einrichtung string to object according to type
        /// </summary>
        /// <param name="entry">config entry with einrichtung string and type</param>
        public static object GetEinrichtungObject(WfvConfigEntry entry)
        {
            object einrichtungObj = null;
            ...
            if (typ.Equals("CAS"))
            {
                ...
                einrichtungObj = converter.ConvertTo(entry.einrichtung, typeof(iCASEvaluateDto));
            }
            else if (typ.Equals("WORKFLOW"))
            {
                XmlObjectConverter<Activity> converter = new XmlObjectConverter<Activity>();
                einrichtungObj = converter.ConvertTo(entry.einrichtung, typeof(Activity));
            }
            ...
            else
            {
                ...
                if (entry.einrichtung == null || entry.einrichtung.Length == 0)
                    einrichtungObj = new WfvEntry();
                else
                    einrichtungObj = converter.ConvertTo(entry.einrichtung, typeof(WfvEntry));
            }

            return einrichtungObj;
        }
        ...
        public void SetData(object newElement)
        {
            if (ContentData == null)
                return;
            ...
            if (newElement is WfvConfigEntryDto)
            {
                ...
                if (ConfigEntrySyscodes.Contains(entry.syscode))
                {
                    WfvConfigEntryDto oldEntry = ConfigEntryList.Find((a)=>a.syscode == entry.syscode);
                    ConfigEntryList.Remove(oldEntry);
                }
                else
                {
                    RegisterEntry(entry);
            ...
            else if (newElement is WfvEntry)
            {
                ...
                if (syscodes.ContainsKey(entry.syscode))
                {
                    WfvEntry oldEntry = ContentData.entries.Find((a) => a.syscode == entry.syscode);
                    ContentData.entries.Remove(oldEntry);
                }
                else
                {
        ...
        private static void RegisterEntry(WfvEntry entry)
        {
            ...
            {
                log.Debug("syscode null (WfvEntry)");
                return;
            }
        ...
        {
            if (entry == null || entry.syscode == null)
                return;
        ...
        private static void RegisterEntry(WfvConfigEntryDto entry)
        {
            ...
            if (entry.syscode == null)
            {
                log.Debug("syscode null (WfvConfigEntry");
                return;
            }
