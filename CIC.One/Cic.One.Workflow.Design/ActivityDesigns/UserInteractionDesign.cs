...
namespace Cic.One.Workflow.Design.ActivityDesigns
{
    ...
    public class UserInteraction : IActivityTemplateFactory
    {
        public Activity Create(DependencyObject target)
        {
            
            
            return new Cic.One.Workflow.Activities.UserInteraction
            {
                wfcontext = new InArgument<WorkflowContext>(new VisualBasicValue<WorkflowContext>("input")),
                //Result = new OutArgument<WorkflowContext>(new System.Activities.Variable<WorkflowContext>("input"))
            };
        }
    }
}
